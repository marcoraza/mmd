...
type ParsePackingImportFileResult =
  { ok: true; data: PackingImportRowInput[] } | { ok: false; error: string }
...
function rowsToPackingImportRows(rows: unknown[][]): ParsePackingImportFileResult {
  ...
  if (!mappedHeaders.some(Boolean)) {
    return {
      ok: false,
      error:
        'Cabeçalho não reconhecido. Use codigo_interno, categoria, item, subcategoria, marca, modelo, quantidade e observacao.',
    }
  }
...
function normalizeExcelCellValue(value: unknown): PackingImportCellValue {
  ...
  if (Array.isArray(record.richText)) {
    return record.richText
      .map((part) =>
        typeof part === 'object' && part ? String((part as { text?: unknown }).text ?? '') : '',
      )
      .join('')
  }
  return ''
