...
function dashboardProjectSource(project: Projeto, detail: ProjectDetail | null): DashboardProjectSource {
  if (detail) {
    return {
      id: detail.id,
      nome: detail.nome,
  ...
  const entries = await Promise.all(
    relevant.map(async (project) => {
      try {
        ...
        console.warn(`[mmd-dashboard] detalhe indisponível para ${project.id}`, error)
