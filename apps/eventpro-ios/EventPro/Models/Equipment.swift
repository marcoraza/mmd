...
enum Categoria: String, Codable, CaseIterable, Identifiable {
    ...
    /// Prefixo do código interno. Continua `ILU`, `AUD`, ... porque as
    /// etiquetas físicas (`MMD-ILU-0001`) já estão coladas nos equipamentos.
    var prefix: String {
        switch self {
...
/// Espelha a tabela `items` do Supabase.
...
extension Equipment {
    ...
    /// "Marca Modelo" quando existir, senão o nome do item.
    var displayName: String {
        let parts = [marca, modelo].compactMap { $0 }
        return parts.isEmpty ? nome : parts.joined(separator: " ")
    }

    /// Valor de mercado formatado, ex.: "R$ 1.200,00".
