import type { StatusLote, StatusSerial } from '@/lib/types'

export type PublicAssetStatus = 'Ativo' | 'Em uso' | 'Indisponível' | 'Falar com MMD'
...
export function serialStatusToPublicStatus(status: StatusSerial): PublicAssetStatus {
  ...
  return 'Falar com MMD'
}

export function loteStatusToPublicStatus(status: StatusLote): PublicAssetStatus {
  if (status === 'DISPONIVEL') return 'Ativo'
  if (status === 'EM_CAMPO') return 'Em uso'
  return 'Indisponível'
}
...
export function getPublicQrContact(env: Record<string, string | undefined> = process.env) {
  const whatsappUrl = env.NEXT_PUBLIC_MMD_WHATSAPP_URL?.trim() || null
  const phone = env.NEXT_PUBLIC_MMD_PHONE?.trim() || null
