...
export type EventImportReviewStatus = 'IMPORTADO_OFICIAL' | 'REVISAO_PENDENTE' | 'REVISAO_CONCLUIDA'
...
export function parseEventDate(value: unknown, fallbackYear = 2026): string | null {
  ...
  const monthName = text.match(
    /\b(\d{1,2})\s*(JAN|JANEIRO|FEV|FEVEREIRO|MAR|MARCO|ABR|ABRIL|MAI|MAIO|JUN|JUNHO|JUL|JULHO|AGO|AGOSTO|SET|SETEMBRO|OUT|OUTUBRO|NOV|NOVEMBRO|DEZ|DEZEMBRO)\b/,
  )
  if (monthName) {
...
export function classifyEventProLine(value: unknown): EventProLineClassification {
  ...
  if (!normalizedName)
    return { kind: 'empty', normalizedName, displayName, categoriaSugerida: null }
