...
def fetch_url(url: str, retries: int = 3, sleep_s: float = 0.8) -> str:
...
def infer_from_known_items(item: ItemRow, known_values: list[tuple[ItemRow, float]]) -> SourceMatch | None:
    ...
    subcat = fold_text(item.subcategoria)
    category = fold_text(item.categoria)

    same_brand_subcat = [value for other, value in known_values if fold_text(other.marca) == brand and fold_text(other.subcategoria) == subcat]
    same_subcat = [value for other, value in known_values if fold_text(other.subcategoria) == subcat]
    ...
    for label, candidates, confidence in [
        ...
        ("estimativa_subcategoria", same_subcat, 0.42),
        ("estimativa_categoria", same_category, 0.30),
    ]:
        minimum = 1 if label == "estimativa_brand_subcategoria" else (2 if label == "estimativa_subcategoria" else 5)
