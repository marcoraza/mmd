...
test('roleAllows applies the EventPro write hierarchy', () => {
...
test('operatorLabel prefers profile name then email then stable fallback', () => {
  assert.equal(
    operatorLabel({ nome: ' Marcelo Santos ', email: 'marcelo@eventpro.test' }),
    'Marcelo Santos',
  )
  assert.equal(operatorLabel({ email: ' equipe@eventpro.test ' }), 'equipe@eventpro.test')
  assert.equal(operatorLabel({ fallbackId: 'user-1' }), 'user-1')
  assert.equal(operatorLabel({}), 'Usuário EventPro')
})
