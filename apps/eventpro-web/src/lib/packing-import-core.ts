...
export type PackingImportRow =
  PackingImportMatchedRow | PackingImportAmbiguousRow | PackingImportInvalidRow
...
export function normalizePackingImportHeader(
  value: PackingImportCellValue,
): PackingImportColumn | null {
...
export function buildPackingImportApprovals(
  ...
  const resolutionMap = new Map(
    resolutions.map((resolution) => [resolution.rowNumber, resolution.itemId]),
  )
  ...
  return [...approvalsByItem.values()].sort((a, b) => a.sourceRowNumbers[0] - b.sourceRowNumbers[0])
}
...
function matchPackingImportRow(
  ...
  if (code) {
    ...
    if (!match) {
      errors.push('Código interno não encontrado no catálogo.')
      return {
        ...base,
        status: 'invalid',
        matchReason: null,
  ...
  if (!valueText(row.categoria)) errors.push('Categoria obrigatória quando não há código interno.')
  else if (!category) errors.push('Categoria não reconhecida.')
  if (!itemText) errors.push('Item obrigatório quando não há código interno.')
...
function matchesItemText(candidate: PackingImportCandidate, itemText: string, tokens: string[]) {
