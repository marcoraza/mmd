...
export function createPackingSuggestionWorkspace({
  ...
  const templateSuggestions = templates
    ...
    .slice(0, 6)
    .map((template): PackingSuggestionCandidate => ({
...
function scoreHistoricalProject(
  target: PackingSuggestionProject,
...
function historicalReason(
  ...
  if (
    normalizeText(target.cliente) &&
    normalizeText(target.cliente) === normalizeText(project.cliente)
  ) {
