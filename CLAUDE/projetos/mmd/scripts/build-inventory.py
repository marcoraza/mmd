...
STATUS_STYLE = {
    'DISPONIVEL': {'bg':'DCFCE7','tx':'166534'},
    'PACKED':     {'bg':'DBEAFE','tx':'1E40AF'},
    'EM_CAMPO':   {'bg':'FEF9C3','tx':'854D0E'},
    'MANUTENCAO': {'bg':'FEE2E2','tx':'991B1B'},
    'EMPRESTADO': {'bg':'EDE9FE','tx':'5B21B6'},
    'VENDIDO':    {'bg':'F1F5F9','tx':'64748B'},
    'BAIXA':      {'bg':'F1F5F9','tx':'64748B'},
}
ESTADO_STYLE = {
    'NOVO':           {'bg':'DCFCE7','tx':'166534'},
    'SEMI_NOVO':      {'bg':'DBEAFE','tx':'1E40AF'},
    'USADO':          {'bg':'FEF9C3','tx':'854D0E'},
    'RECONDICIONADO': {'bg':'FFEDD5','tx':'9A3412'},
}
DESGASTE_BG = {5:'DCFCE7',4:'DBEAFE',3:'FEF9C3',2:'FFEDD5',1:'FEE2E2'}
...
def add_cond_fmt(ws, start_row, end_row, status_col='G', estado_col='H', desgaste_col='I'):
...
def build_manual(wb):
    ...
    def section(row, title):
        ...
        ws.cell(row=row, column=2).font = FN(13, True, DARK)
        ws.cell(row=row, column=2).fill = BG(BG_ALT)
        ws.cell(row=row, column=3).fill = BG(BG_ALT)
        ws.row_dimensions[row].height = 32

    def line(row, text, color=MID, bold=False, indent=False, bg_color=None):
        col = 3 if indent else 2
        if not indent:
            ws.merge_cells(f'B{row}:C{row}')
        ws.cell(row=row, column=col).value = text
        ws.cell(row=row, column=col).font = FN(10, bold, color)
        if bg_color:
            ws.cell(row=row, column=col).fill = BG(bg_color)
        ws.row_dimensions[row].height = 22

    def color_example(row, label, bg_c, tx_c):
        ws.cell(row=row, column=2).value = label
        ws.cell(row=row, column=2).font = FN(10, True, tx_c)
        ws.cell(row=row, column=2).fill = BG(bg_c)
    ...
    ws.cell(row=r, column=2).value = 'INVENTARIO MMD EVENTOS'
    ws.cell(row=r, column=2).font = FN(18, True, DARK)
    ws.row_dimensions[r].height = 40
    r += 1
    ws.merge_cells(f'B{r}:C{r}')
    ws.cell(row=r, column=2).value = 'Manual de Uso da Planilha'
    ws.cell(row=r, column=2).font = FN(11, False, ACCENT)
    r += 2

    section(r, 'NAVEGACAO'); r += 1
    line(r, 'Cada aba na parte inferior = uma categoria de equipamento.'); r += 1
    line(r, 'DASHBOARD mostra o resumo geral com graficos e numeros.'); r += 1
    line(r, 'Use os filtros no header de cada coluna para buscar.'); r += 1
    line(r, 'LOTES agrupa cabos em kits. FORA DE OPERACAO = vendidos/emprestados.'); r += 2

    section(r, 'STATUS (onde o item esta)'); r += 1
    for st, label in [('DISPONIVEL','No galpao, pronto'),('EM_CAMPO','Em evento'),('PACKED','Separado pro evento'),
                       ('MANUTENCAO','Em reparo'),('EMPRESTADO','Com terceiro'),('VENDIDO','Vendido'),('BAIXA','Descartado')]:
        color_example(r, st, STATUS_STYLE[st]['bg'], STATUS_STYLE[st]['tx'])
        ws.cell(row=r, column=3).value = label
        ws.cell(row=r, column=3).font = FN(10, False, LIGHT)
        r += 1
    r += 1

    section(r, 'ESTADO (ciclo de vida)'); r += 1
    for est, label in [('NOVO','Nunca usado em evento'),('SEMI_NOVO','Usado 1-5 vezes'),
                        ('USADO','Uso regular'),('RECONDICIONADO','Reparado/restaurado')]:
        color_example(r, est, ESTADO_STYLE[est]['bg'], ESTADO_STYLE[est]['tx'])
        ws.cell(row=r, column=3).value = label
        ws.cell(row=r, column=3).font = FN(10, False, LIGHT)
        r += 1
    r += 1

    section(r, 'DESGASTE (condicao fisica, 1 a 5)'); r += 1
    for d, label in [(5,'Excelente'),(4,'Bom'),(3,'Regular'),(2,'Desgastado'),(1,'Critico')]:
        color_example(r, str(d), DESGASTE_BG[d], DARK)
        ws.cell(row=r, column=3).value = label
        ws.cell(row=r, column=3).font = FN(10, False, LIGHT)
        r += 1
    r += 1

    section(r, 'DEPRECIACAO (automatica)'); r += 1
    line(r, 'Valor Atual = Valor Original x (Desgaste/5) x Fator do Estado'); r += 1
    line(r, 'NOVO=100%  |  SEMI_NOVO=85%  |  USADO=65%  |  RECONDICIONADO=50%', LIGHT); r += 1
    line(r, 'Mude o Desgaste ou Estado e o Valor Atual recalcula sozinho.', ACCENT, True); r += 2

    section(r, 'COMO ATUALIZAR'); r += 1
    line(r, 'Item novo: adicionar na aba da categoria. Estado=NOVO, Desgaste=5.'); r += 1
    line(r, 'Apos evento: revisar desgaste dos itens que foram.'); r += 1
    line(r, 'Defeito: status=MANUTENCAO, ajustar desgaste.'); r += 1
    line(r, 'Vendido: mover para aba FORA DE OPERACAO.'); r += 2

    section(r, 'CATEGORIAS'); r += 1
    for cat in CAT_ORDER:
        d = CAT_DESIGN[cat]
        ws.cell(row=r, column=2).value = f"{cat} ({d['prefix']})"
        ws.cell(row=r, column=2).font = FN(10, True, d['tx'])
        ws.cell(row=r, column=2).fill = BG(d['bg'])
        ws.cell(row=r, column=2).border = BD()
        ws.cell(row=r, column=3).value = d['desc']
        ws.cell(row=r, column=3).font = FN(9, False, LIGHT)
...
def build_dashboard(wb, activos, lots):
    ...
    deprec_pct = (val_atual/val_orig*100) if val_orig > 0 else 0
...
def build_cat_tab(wb, cat, items):
    ...
    for subcat in sorted(by_sub.keys()):
        ...
        for item in sub_items:
            ...
            # Desgaste (numerico pra formulas, conditional formatting colore)
            ...
            if item['valor_mercado']:
                ...
                ws.cell(row=r, column=12).value = f'=IF(J{r}>0,K{r}/J{r},"")'
