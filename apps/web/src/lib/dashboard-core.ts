...
function splitHeroTitle(project: DashboardProjectSource): Pick<
  DashboardHeroEvent,
...
> {
  ...
  return {
    title_line1: name,
    title_line2: project.cliente?.trim() || project.local?.trim() || 'Evento MMD',
  }
}
...
export function buildDashboardData(input: BuildDashboardInput): DashboardData {
  ...
  return {
    ...
    notifications: ordered.filter((project) => eventStatus(project, projectReadiness(project)) !== 'pronto')
      .length,
