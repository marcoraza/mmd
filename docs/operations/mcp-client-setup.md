...
Antes do deploy, execute `npm --prefix apps/web run smoke:mcp-db` com esse secret. O smoke abre uma conexão real pelo mesmo driver da rota, confirma `current_user` e `session_user`, prova que `SELECT public.items` recebe `42501` e que as nove RPCs de leitura são alcançáveis e falham fechadas com token inválido. Rotacione a senha com outro `ALTER ROLE`, atualize o secret e repita o smoke. Em incidente, remova o secret, rode `ALTER ROLE mmd_mcp_executor NOLOGIN PASSWORD NULL;` e revogue o cliente no registry.
...
- coleções paginadas de Eventos e catálogo
- packing, Conferências por direção, movimentações, retorno esperado e pendências por Evento
- `mmd_consultar_evento({ evento_id })`

Os templates exatos, campos e limites estão no catálogo. Todos os recursos operacionais usam `page` a partir de 1 e `page_size` entre 1 e 50 no URI.
