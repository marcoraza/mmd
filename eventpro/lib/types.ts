...
// Status de cobertura de uma linha de packing (próprio + aluguel avulso).
// No legado morava em `src/lib/data/projects.ts`; aqui é tipo de domínio puro,
// consumido por `external-rental-core`.
...
export interface PackingList {
  ...
  item_id: string
  quantidade: number
  // Shape em memória mantido do legado. No EventPro a fonte dos seriais alocados
  // é a tabela relacional `packing_allocations`, não uma coluna `uuid[]`.
